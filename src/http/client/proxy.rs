...
use http::client::error::{Error, Result};
...
/// Returns a `Proxy` from either the `http_proxy` or `https_proxy` environment variable if
...
fn parse_proxy_url(url: &str) -> Result<Option<Proxy>> {
    ...
    Ok(Some(try!(Proxy::new(url, auth))))
}
